...
// TODO!!! ASSERT_SAFE_EXPRESSIONS em tudo
//---------------------------------------------------------------------------
...
void til::xml_writer::do_print_node(til::print_node *const node, int lvl) {
    ...
    oss << "isPrintln=\"" << (node->newline() ? "true" : "false") << "\"";
    openTag(node, lvl, oss.str());
...
void til::xml_writer::do_stop_node(til::stop_node *const node, int lvl) {
    ...
    selfClosingTag(node, lvl, oss.str());
}
...
void til::xml_writer::do_variable_declaration_node(til::variable_declaration_node *const node, int lvl) {
    std::string type = node->type() ? node->type()->to_string() : "unknown";

    std::ostringstream oss;
    oss << "qualifier=\"" << qualifierText(node->qualifier()) << "\""
        << " type=\"" << type << "\""
        << " identifier=\"" << node->identifier() << "\"";

    openTag(node, lvl, oss.str());
    if (node->initializer()) { // Variavel declarada com inicializacao
...
void til::xml_writer::do_function_node(til::function_node *const node, int lvl) {
    ...
    std::ostringstream oss;
    oss << "isMain=\"" << (node->is_main() ? "true" : "false") << "\""
        << " type=\"" << type << "\"";
